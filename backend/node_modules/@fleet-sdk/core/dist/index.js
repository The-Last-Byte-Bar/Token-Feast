...
var BoxSelector = class {
  ...
  select(target) {
    ...
    if (predicate) {
      selected = inclusion ? unselected.filter((box) => predicate(box) || inclusion.has(box.boxId)) : unselected.filter(predicate);
    } else if (inclusion) {
    ...
    if (common.some(selected)) {
      ...
      if (common.some(remaining.tokens) && selected.some((input) => !common.isEmpty(input.assets))) {
        for (const t of remaining.tokens) {
          if (t.amount && t.amount > common._0n) t.amount -= common.utxoSum(selected, t.tokenId);
        }
      }
    }
    if (this._selector) {
...
var MAX_TOKENS_PER_BOX = 100;
var TokensCollection = class extends common.Collection {
  #minting;
  constructor(tokens, options) {
    super();
    if (common.isDefined(tokens)) this.add(tokens, options);
  }
  get minting() {
  ...
  add(items, options) {
    if (Array.isArray(items)) {
      if (items.some((x) => !x.tokenId)) throw new common.FleetError("TokenID is required.");
  ...
  mint(token) {
    if (common.isDefined(this.#minting)) {
      throw new common.FleetError("Only one minting token is allowed per transaction.");
    }
...
function validateUnpackedAddress(unpacked) {
  ...
  if (unpacked.type === common.AddressType.P2PK && !crypto.validateEcPoint(unpacked.body)) return false;
...
var ErgoAddress = class _ErgoAddress {
  ...
  static fromHash(hash, network) {
    ...
    const ergoTree = common.concatBytes(P2SH_ERGOTREE_PREFIX, bytes, P2SH_ERGOTREE_SUFFIX);
    return new _ErgoAddress(ergoTree, network);
  }
  /**
  ...
  static decodeUnsafe(encodedAddress) {
    return _ErgoAddress.#fromUnpacked(unpackAddress(crypto.base58.decode(encodedAddress)));
  }
...
var ErgoUnsignedTransaction = class {
  ...
  get burning() {
    const diff = common.utxoDiff(common.utxoSum(this.inputs), common.utxoSum(this.outputs));
...
var InputsCollection = class extends common.Collection {
  constructor(boxes) {
    super();
    if (common.isDefined(boxes)) this.add(boxes);
  }
  _map(input) {
...
var OutputsCollection = class _OutputsCollection extends common.Collection {
  constructor(outputs) {
    super();
    if (common.isDefined(outputs)) this.add(outputs);
  }
  _map(output) {
    return output;
  }
...
var ErgoMessage = class _ErgoMessage {
  ...
  static decode(encodedHash) {
    ...
    if (unpacked.type !== common.AddressType.ADH) throw new Error("Invalid message type");
...
var OutputBuilder = class {
  ...
  setAdditionalRegisters(registers) {
    ...
    if (!common.areRegistersDenselyPacked(hexRegisters)) throw new InvalidRegistersPacking();
    ...
    if (this.minting) {
      ...
      if (common.isEmpty(this.additionalRegisters)) {
        this.setAdditionalRegisters({
          ...
          R6: serializer.SColl(serializer.SByte, crypto.utf8.decode(this.minting.decimals?.toString() || "0"))
        });
      }
    } else {
...
function createPluginContext(builder) {
  return {
    ...
    addDataInputs: (dataInputs, options) => builder.withDataFrom(dataInputs, options).dataInputs.length,
    burnTokens: (tokens) => {
...
var TransactionBuilder = class {
  ...
  from(inputs, options = { ensureInclusion: false }) {
  ...
  #ensureInclusionOf(inputs) {
    if (!this._ensureInclusion) this._ensureInclusion = /* @__PURE__ */ new Set();
  ...
  configureSelector(selectorCallback) {
    if (common.isUndefined(this._selectorCallbacks)) this._selectorCallbacks = [];
  ...
  build() {
    ...
    if (common.isDefined(this._changeAddress)) {
      ...
      const manualMintingTokenId = target.tokens.some((x) => x.tokenId === firstInputId) ? firstInputId : void 0;
  ...
  _isMinting() {
    return this._getMintingOutput() !== void 0;
  }
  _getMintingTokenId() {
...
function estimateChangeSize({
  ...
  for (let i = 0; i < neededBoxes; i++) size += serializer.estimateVLQSize(baseIndex + i);
